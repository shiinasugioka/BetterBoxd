    ...
    var body: some View {
        NavigationView {
            if horizontalSizeClass == .compact {
                OnboardingScreen(profile: $profile, currentStep: $currentStep) // iPhone layout
            } else {
                HStack {
                    Sidebar(profile: $profile) // Sidebar for iPad
                    OnboardingScreen(profile: $profile, currentStep: $currentStep) // Main content for iPad
                }
            }
...
struct OnboardingScreen: View {
    ...
    var body: some View {
        VStack {
            switch currentStep {
            case .welcome:
                IntroScreenView(currentStep: $currentStep, horizontalSizeClass: horizontalSizeClass)
            case .username:
                UserInputView(profile: $profile, currentStep: $currentStep)
            case .bio:
                BioInputView(profile: $profile, currentStep: $currentStep)
